...
The status pages refresh every 100 ms from the post-filter forward-power ADC reading. The primary page presents a smoothed RMS or PEP peak-hold value on row one and a full-width bracketed PEP bar on row two. A second page presents `PEP [----------]` on row one with temperature in degrees C on row two. Each bar uses all remaining horizontal columns after its label and brackets. The PEP hold decays by one watt per configured interval, adjustable from 50 to 2000 ms and defaulting to 500 ms.
