...
The status page refreshes every 100 ms from the post-filter forward-power ADC reading. The primary value is selectable as a smoothed RMS display or a PEP peak-hold display. The post-filter forward PEP is always presented with an eight-segment bar, scaled to that bridge's configured full-scale power. The PEP hold decays by one watt per configured interval, adjustable from 50 to 2000 ms and defaulting to 500 ms.
