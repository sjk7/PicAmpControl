...
3. **The log tab OPENS on every run - a hard REQUIREMENT (2026-09-26), and it may be the current tab.**
   ...
   same applies to scrolling, and the rule is now explicit: **the extension OWNS the viewport** - the
   one reveal is `followIfFull()` in `tools/simshow/extension.js`, and it scrolls only when the pane
   is FULL, i.e. the whole log no longer fits from the top. While everything fits NOTHING is scrolled
   (the operator rejected the earlier version that followed from the first line: *"only autoscroll
   when the pane is full"*, 2026-09-26). `entry.following` - the permission to follow - is turned off
   ONLY by the view moving **UP** (a real scroll-away by the operator, including to the very top,
   which must not be yanked back), and back on whenever the tail is on screen. Do NOT derive it from
   "tail visible" alone: the log growing past the pane takes the tail off screen WITHOUT any scroll,
   so that test switched following off at exactly the moment it should have started - the log grew to
   32 lines and never followed (found from `%TEMP%/picampcontrol_follow.trace.log`, 2026-09-26).
   **Autoscroll changes only take effect after a reinstall + a window reload, so TEST AUTOSCROLL IN A
   NEW SESSION** - a window still running the old copy shows the old behaviour and reads as "the fix
   failed". (`tools/simulate/prove_autoscroll.py` appends a line every 0.5 s to a scratch log; the
   extension writes its own decisions to `%TEMP%/picampcontrol_follow.trace.log`.)
   ...
   **The helper is shared across EVERY Insiders window, so only ONE window may act on a request.**
   ...
   windows apart. The gate that works is the workspace `root`: `open_progress_log.py` writes `"pid"`
   and `"root"`, and the extension rejects a request when its `root` does not match a workspace
   folder. (The `pid` compare is kept only as a cheap early-out for single-main-per-window layouts.)
   **The "opened in the wrong window / duplicated in my other instance" trouble of 2026-09-26 was NOT
   the PID at all - it was two marketplace log-follower extensions, `berublan.vscode-log-viewer` and
   `log-follower.log-follower`. Both are now UNINSTALLED and must NOT be reinstalled**: each followed
   and scrolled the log by itself, in every window. A `vscode.window.state.focused` gate was tried as
   a fix and rejected too - it blocked runs whenever the window was not focused, and "it should
   autoscroll whether focused or not". Reinstall with `install.ps1` + reload EVERY window after
