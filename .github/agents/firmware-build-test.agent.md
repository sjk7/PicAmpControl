...
- ONLY use the existing repo scripts (`run_tests.sh`, `tools/simulate/*`) and the commands documented in the skill instead of inventing new build commands. Note that the workspace build tasks (`Build PicAmpControl (Debug)`, `Build PicAmpControl (Release)`) are Windows-only - they hard-code `E:/hamcode/...` paths - so do not invoke them on this macOS checkout.

## Approach
1. Determine which target is relevant (Debug, Release, or sim) from the user's request or the file context.
2. Run the appropriate build via the skill's documented `cmake` command (the workspace tasks are Windows-only), capturing output to a log file so warnings and the program-memory figure can be read without flooding the terminal.
